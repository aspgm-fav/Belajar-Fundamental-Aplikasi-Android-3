...
class RepositoryViewModel: ViewModel() {
    ...
    val listRepository = MutableLiveData<ArrayList<User>>()
            ...
            override fun onSuccess(statusCode: Int, headers: Array<out Header>?,
                ...
                try {
                    ...
                    for (i in 0 until responsArray.length()){
                        ...
                        repoItem.name = responObject.getString("name")
                        val owner = responObject.getJSONObject("owner")
                        repoItem.avatar = owner.getString("avatar_url")

                        listItems.add(repoItem)
                    }
                    listRepository.postValue(listItems)
                } catch (e: Exception){
    ...
    fun getRepository(): LiveData<ArrayList<User>> {
